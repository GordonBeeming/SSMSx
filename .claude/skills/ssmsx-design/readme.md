...
- **Color usage:** accent blue is reserved for primary actions, focus, selection and SQL keywords. Status colors are semantic and consistent everywhere — green = success/run, red = error/destructive, amber = cancelled/warning, a bright `#4ade80` = "server online" dots. **Connection colour profiles** are a signature: every saved connection requires a background/foreground pair (red prod, green local…). Active connection surfaces use the complete pair; inactive surfaces preserve the profile's foreground as a compact marker across the toolbar, tree, tabs and status bar.
...
- **Radius:** restrained. 4px on buttons/inputs/tabs, 6px on grouped cards, 8px on the modal dialog, full-round only on compact connection-profile markers. The **app icon** is the one place with generous rounding (~22% squircle).
...
- **Selection / active:** tree selection is accent at **15% tint** (`color-mix`). Active connection-owned query and result tabs use their complete profile background/foreground pair; inactive tabs show the foreground marker. Dialog tabs get a 2px accent bottom-border.
...
- **Status as iconography:** small **coloured markers** carry a lot of meaning — green = online server, and the foreground marker of each connection profile. Unicode glyphs stand in for toolbar affordances: `▶` execute, `▶|` execute-selection, `■` cancel, `•` dirty/unsaved, `×` close.
