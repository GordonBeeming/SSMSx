...
function ResultsGrid({ tab, onTab, connection }) {
  const data = window.SSMSX_DATA;
  const resultTab = (key, label) => {
    const active = tab === key;
    return (
      <button onClick={() => onTab(key)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", border: "none", background: active ? connection.profileBackground : "transparent", cursor: "pointer", color: active ? connection.profileForeground : "var(--text-secondary)" }}>
        <span style={{ width: 8, height: 8, borderRadius: "var(--radius-full)", background: connection.profileForeground, flexShrink: 0 }} />
        {label}
      </button>
    );
  };
  return (
    <div style={{ display: "flex", flexDirection: "column", overflow: "hidden", borderTop: "1px solid var(--border-default)", maxHeight: "46%" }}>
      <div style={{ display: "flex", flexWrap: "wrap", background: "var(--surface-panel)", borderBottom: "1px solid var(--border-default)", fontSize: "var(--text-xs)" }}>
        {resultTab("results", `Results (${data.rowsAffected})`)}
        {resultTab("messages", "Messages")}
      </div>
      {tab === "results" ? (
        <div style={{ overflow: "auto" }}>
          <table style={{ width: data.columns.length * 160 + 42, tableLayout: "fixed", borderCollapse: "collapse", fontSize: "var(--text-xs)", fontFamily: "var(--font-mono)" }}>
            <colgroup><col style={{ width: 42 }} />{data.columns.map((column) => <col key={column} style={{ width: 160, maxWidth: 500 }} />)}</colgroup>
            <thead style={{ position: "sticky", top: 0 }}>
              <tr>
                <th style={thStyle}></th>
                {data.columns.map((c) => <th key={c} title={c} style={thStyle}>{c}</th>)}
              </tr>
            </thead>
            <tbody>
              {data.rows.map((row, ri) => (
                <tr key={ri} className="grid-row">
                  ...
                  {row.map((cell, ci) => <td key={ci} title={String(cell)} style={tdStyle}>{String(cell)}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
...
const thStyle = { maxWidth: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", borderBottom: "1px solid var(--border-default)", borderRight: "1px solid var(--border-default)", padding: "3px 8px", textAlign: "left", fontWeight: 500, color: "var(--text-secondary)", background: "var(--surface-panel)" };
const tdStyle = { maxWidth: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", borderBottom: "1px solid var(--border-default)", borderRight: "1px solid var(--border-default)", padding: "2px 8px", color: "var(--text-primary)" };
...
function Workbench({ connection, onAddConnection, onDisconnect }) {
  ...
  const [tabs, setTabs] = React.useState([{ id: "q1", title: "Query 1", database: "Sales", dirty: true, pinned: true }]);
  ...
  const togglePinned = (id) => setTabs((current) => current.map((tab) => tab.id === id ? { ...tab, pinned: !tab.pinned } : tab));
  ...
  const renderTab = (tab) => (
    <QueryTab key={tab.id} title={tab.title} database={tab.database} active={activeTab === tab.id} dirty={tab.dirty} pinned={tab.pinned}
      profileBackground={connection.profileBackground} profileForeground={connection.profileForeground}
      onTogglePinned={() => togglePinned(tab.id)} onContextMenu={(event) => openTabMenu(event, tab)}
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "var(--surface-app)", color: "var(--text-primary)", fontFamily: "var(--font-ui)" }} onClick={() => setMenu(null)}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, borderBottom: "1px solid var(--border-default)", background: "var(--surface-panel)", padding: "8px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ width: 10, height: 10, borderRadius: "var(--radius-full)", background: connection.profileForeground }} />
          <span style={{ fontSize: "var(--text-sm)" }}>{connection.alias || connection.serverName}</span>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflow: "auto" }}>
            {visible.map((n) => (
              <TreeRow key={n.id} type={n.type} label={n.label} depth={n.depth}
                ...
                profileForeground={n.profileForeground}
        ...
        <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
          {/* tab bar */}
          <div style={{ display: "flex", flexDirection: "column", borderBottom: "1px solid var(--border-default)", background: "var(--surface-panel)" }}>
            {pinnedTabs.length > 0 && <div style={{ display: "flex", flexWrap: "wrap", borderBottom: "1px solid var(--border-default)" }}>{pinnedTabs.map(renderTab)}</div>}
          ...
          {/* results */}
          {executed && <ResultsGrid tab={resultTab} onTab={setResultTab} connection={connection} />}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, borderTop: "1px solid var(--border-default)", background: "var(--surface-panel)", padding: "3px 12px", fontSize: "var(--text-xs)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ width: 8, height: 8, borderRadius: "var(--radius-full)", background: connection.profileForeground }} />
              <span style={{ color: "var(--text-secondary)" }}>{connection.alias || connection.serverName} · {connection.database}</span>
            </div>
            <div style={{ flex: 1 }} />
      ...
      {menu && <ContextMenu x={menu.x} y={menu.y} onClose={() => setMenu(null)} items={[
        { label: menu.tab.pinned ? "Unpin Tab" : "Pin Tab", onClick: () => togglePinned(menu.tab.id) },
        { separator: true },
        { label: "Close", onClick: () => closeTab(menu.tab.id) },
      ]} />}
    </div>
  );
}
